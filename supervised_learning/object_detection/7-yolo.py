...
class Yolo:
    ...
    def __init__(self, model_path, classes_path, class_t, nms_t, anchors):
        ...
        with open(classes_path, 'r') as classes:
            self.class_namess = [line.strip() for line in classes]
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, class_names):
        """
        Show boxes.
        """
        imagec = image.copy()
        for idx, box in enumerate(boxes):
            ...
            class_names = self.class_namess[box_classes[idx]]
            score = box_scores[idx]
            color = (255, 0, 0)
            cv2.rectangle(imagec, (top_left_x, top_left_y),
                          (bottom_right_x, bottom_right_y),
                          color, 2)
            text = class_names + " " + "{:.2f}".format(score)
            cv2.putText(imagec, text, (top_left_x, top_left_y - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1,
                        cv2.LINE_AA)
        cv2.imshow(class_names, imagec)
        key = cv2.waitKey(0)
        if key == ord('s'):
            if not os.path.exists('./detections'):
                os.mkdir('./detections')
            cv2.imwrite(os.path.join('./detections', class_names), imagec)
    ...
    def predict(self, folder_path):
        ...
        image_size = np.array([self.model.input.shape[2],
                               self.model.input.shape[1]])
        boxes, box_conf, box_class_probs = self.process_outputs(outputs,
                                                                image_size)
        flt_box, box_cls, box_scr = self.filter_boxes(boxes,
                                                      box_conf,
                                                      box_class_probs)
        box, prd_box, prd_box_scr = self.non_max_suppression(flt_box,
                                                             box_cls,
                                                             box_scr)
        for i in range(len(images)):
            self.show_boxes(images[i], box[i],
                            prd_box[i], prd_box_scr[i],
                            self.class_namess)
