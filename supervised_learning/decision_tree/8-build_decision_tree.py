...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        ...
        left_proportions = np.array([
            np.mean(target_reduced[feature_values > t] == cls, axis=0)
            for t in thresholds for cls in classes
        ]).reshape(len(thresholds), len(classes))
        right_proportions = np.array([
            np.mean(target_reduced[feature_values <= t] == cls, axis=0)
            for t in thresholds for cls in classes
        ]).reshape(len(thresholds), len(classes))
        ...
        return thresholds[min_index], weighted_gini[min_index]
